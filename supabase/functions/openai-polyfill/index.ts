// Supabase Edge Function to receive and store traces from OpenAI Agents SDK
...
serve(async (req) => {
  ...
  try {
    ...
    // Check if this is the /ingest endpoint (OpenAI format) or legacy endpoint
    const url = new URL(req.url);
    const isIngestEndpoint = url.pathname.endsWith('/ingest');

    if (isIngestEndpoint) {
      // Handle OpenAI Agents SDK format: { data: [ {...trace/span...}, ... ] }
      return await handleIngestRequest(req, supabaseClient, user.id);
    } else {
      // Return 404 for now - we're deprecating the old OTLP endpoint
      return new Response(
        JSON.stringify({ error: 'Endpoint not found. Use /traces/ingest for OpenAI traces.' }),
        {
          status: 404,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        }
 ...
 */
async function handleIngestRequest(req: Request, supabaseClient: any, userId: string) {
  try {
